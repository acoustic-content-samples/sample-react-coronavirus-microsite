...
const Contact = ({styleGuide}) => {
    
    const data = CompanyInfo();

    if(!data?.contactNumber && !data?.emailAddress){
        return null;
    }
    return(
        <div className="sidebarSection">
            <div className="sidebarTitle" style={{color:styleGuide.titleText, fontFamily:styleGuide.fontFamily}}>Contact</div>
                <ul style={{color:styleGuide.bodyText, fontFamily:styleGuide.fontFamily}}>
                    {data?.contactNumber &&
                        <li>{data?.contactNumber.value}</li>
                    }
                    {data?.emailAddress.value &&
                        <li>{data?.emailAddress.value}</li>
                    }
                </ul>
        </div>
